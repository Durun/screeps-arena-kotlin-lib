...
/**
 * @param {WebAssembly.Instance} instance_
 * @param {Uint8Array} heap_
 * @param {string[]} global_arguments_
 */
function notify(instance_, heap_, global_arguments_) {
    instance = instance_;
    heap = heap_;
    global_arguments = global_arguments_;
}
...
function linkDependencies(dependencies) {
    for (const key in dependencies) {
        konan_dependencies.env[key] = dependencies[key];
    }
}
 ...
 */
export function instantiateLinking(wasmBin, dependencies, args) {
    linkDependencies(dependencies);
    return WebAssembly.instantiate(wasmBin, konan_dependencies)
        .then(obj => {
            notify(
                obj.instance,
...
linkJavaScriptLibraries();
export default {instantiateLinking, konan_dependencies};